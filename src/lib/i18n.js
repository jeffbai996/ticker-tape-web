...
const STRINGS = {
  ...
  'alerts.subtitle': {
    en: 'live conditions · one-shot until re-armed',
    zh: '实时条件 · 触发一次后需重置',
  },
  'alerts.none': { en: 'no alerts configured', zh: '尚未配置提醒' },
  'alerts.builder': { en: 'New alert', zh: '新提醒' },
  'alerts.builder.live': { en: 'live feed', zh: '实时行情' },
  'alerts.empty': { en: 'No alerts armed', zh: '暂无已启用提醒' },
  'alerts.empty.hint': { en: 'choose a condition', zh: '选择条件' },
  'alerts.recipe.price': { en: 'Price level', zh: '价格水平' },
  'alerts.recipe.rsi': { en: 'RSI threshold', zh: 'RSI 阈值' },
  'alerts.recipe.volume': { en: 'Volume surge', zh: '成交量异动' },
  'alerts.delivery.title': { en: 'Discord delivery', zh: 'Discord 推送' },
  'alerts.delivery.explain': {
    en: 'Optional · channel + hourly cap',
    zh: '可选 · 频道与每小时上限',
  },
  'alerts.delivery.none': {
    en: 'No Discord channels · browser alerts still work.',
    zh: '暂无 Discord 频道 · 浏览器提醒仍可使用。',
  },
  ...
  'chat.preview_heading': { en: 'Ask about a ticker, a sector, or the market.', zh: '询问股票、板块或市场。' },
  'chat.preview_disabled': {
    en: 'The production interface is shown here, but its model and thread services are disabled on this public build.',
    zh: '此处展示生产版界面，但公开版本不启用模型与对话存储服务。',
  },
...
const LABELS = {
  ...
  Indices: '指数', Aluminum: '铝', 'this week': '本周', 'next week': '下周', later: '之后',
  HSI: '恒指', N225: '日经', KOSPI: '韩综指', SSE: '上证', BTC: '比特币',
  ...
  preview: '预览', 'preview only': '仅预览', 'operator workspace': '操作台', context: '上下文', service: '服务',
  'disabled in public demo': '公开演示版中已停用', 'No request will be sent.': '不会发送任何请求。',
  'model unavailable': '模型不可用', 'example prompts': '示例问题',
  'AI generation is disabled in the public demo. No request will be sent.':
    '公开演示版中 AI 生成功能已停用，不会发送任何请求。',
  'live quotes': '实时行情', technicals: '技术指标', watchlist: '自选股', alerts: '提醒', memory: '记忆',
  'what is moving today?': '今天哪些标的在异动？',
  'what is on the calendar this week?': '本周日历上有什么？',
  'compare two companies': '比较两家公司', 'explain a technical signal': '解释一个技术信号',
  ...
  Performance: '净值', 'since last': '较上次', Ledger: '账本', line: '行', Events: '分红财报', 'vs indices': '指数对比', 'FX impact': '汇率影响', Upcoming: '即将到来', 'HK stocks': '港股', 'A-shares': 'A股', 'US stocks': '美股', Canada: '加拿大', 'limit up': '涨停', 'limit down': '跌停', 'market open': '交易中', 'market lunch': '午休', 'market pre': '盘前', 'market closed': '休市', HSI: '恒指', 'CSI 300': '沪深300', 'S&P 500': '标普500', 'This book': '本组合', 'Day move': '当日涨跌', 'of book': '占比', 'Rate': '汇率', 'No currency exposure — everything trades in the display currency.': '无汇率敞口 — 所有持仓均以显示货币计价。', 'Nothing due in the next 30 days.': '未来30天没有安排。', 'results': '财报', 'ex-div': '除息', 'Next 30 days': '未来30天', Today: '今天', Yesterday: '昨天', 'Open original': '打开原文', Close: '关闭', Reading: '阅读中', 'Pick a story to read it here.': '点选一条新闻在此阅读。', 'Could not load the story — open the original instead.': '无法加载正文 — 请打开原文。', 'cloud offline': '云端离线', 'Search news': '搜索新闻', 'Show more': '显示更多', 'No matching headlines.': '没有匹配的新闻。', Held: '持有', 'Next results': '下次财报', 'Ex-dividend': '除息日', 'Last dividend': '最近分红', 'Est. income': '预计收入', 'Upcoming 14 days': '未来14天', 'No dates yet': '暂无日期', 'per share': '每股', Yield: '股息率', 'Nothing scheduled in the next two weeks.': '未来两周没有安排。', 'paid': '已派', 'record': '登记日',
  ...
  // wave-3 sweep (2026-08-11): wire recovery, drag reorder, alert editing,
  // chart drawings, session-countdown/status strings
  'reconnecting on its own — click to try now': '正在自动重连——点击立即重试',
  'quotes stopped updating — the feed is not answering': '行情已停止更新——数据源无响应',
  'drag to reorder': '拖动排序', 'from the board': '移出看盘',
  collapse: '收起', intraday: '日内',
  'avg move': '平均涨跌', '±2% movers': '涨跌超2%',
  'symbol required': '请填写代码', 'operator must be > or <': '条件必须为 > 或 <',
  'value must be a number': '数值必须为数字', 'RSI must be 0-100': 'RSI 需在 0-100 之间',
  'SMA window must be an integer ≥ 2': 'SMA 周期须为不小于 2 的整数',
  'unknown alert type': '未知的提醒类型',
  'editing alert': '编辑提醒', 'Save alert': '保存提醒',
  'transcribing…': '转写中…', 'transcription failed: {err}': '转写失败：{err}',
  ...
  LINE: '水平线', TREND: '趋势线', DELETE: '删除', CLEAR: '清除',
  'price line — tap the chart to drop a horizontal line at that price': '水平价格线 — 点击图表在该价位画一条横线',
  ...
  // Event workspace (2026-08-18): the calendar row opens into a full panel, so
  // the plain-language lines and the link reasons are UI copy now, not tooltips.
  'Event workspace': '事件工作台', 'back to calendar': '返回日历',
  'time to event': '距事件', 'since release': '发布后', 'What it is': '这是什么',
  'Why it matters': '为何重要', 'Affected sectors': '受影响板块',
  'Linked symbols': '关联标的', 'Prior': '前值', Consensus: '预期', Actual: '实际',
  Surprise: '意外偏离', 'awaiting release': '等待发布', released: '已发布',
  ...
  'long duration prices the rate path': '长久期债券反映利率路径',
  'the 10-year yield itself': '10年期收益率本身',
  'banks trade the curve': '银行股交易收益率曲线',
  'property is a rates instrument': '地产本质上是利率资产',
  'homebuilders trade the mortgage rate': '房建股跟随房贷利率',
  'gold trades real rates and the dollar': '黄金交易实际利率与美元',
  'industrial margins live on input costs': '工业利润率取决于原料成本',
  'materials sit at the top of the cost chain': '材料位于成本链最上游',
  'small caps carry domestic growth': '小盘股代表本土增长',
  'retail is the direct read': '零售是最直接的读数',
  'discretionary spending shows up here first': '可选消费最先反映支出变化',
  'staples are the defensive side of the same trade': '必需消费是同一交易的防守端',
  'freight moves with real goods demand': '货运随实物需求波动',
  'services demand runs through media and telecom': '服务需求经由传媒与电信体现',
  'the dollar is the first thing policy moves': '美元最先反映政策变化',
  'the name the event is about': '事件直接指向的标的',
  'its sector proxy reprices with it': '所属板块会同步重定价',
  // sectors
  Rates: '利率', Gold: '黄金', 'Real Estate': '房地产', Homebuilders: '房屋建筑',
  'Broad market': '大盘', 'Small caps': '小盘股', Cyclicals: '周期股',
  Consumer: '消费', Retail: '零售', Transport: '运输', Services: '服务业',
  Volatility: '波动率', 'Single name': '个股', 'Sector peers': '同业板块',
  Suppliers: '供应链', 'Policy-sensitive sectors': '政策敏感板块',
  // plain-language descriptions
  'The Federal Reserve sets the policy rate and explains its thinking.': '美联储决定政策利率并说明其判断依据。',
  'The rate path reprices every other asset, so the statement moves more than the decision.': '利率路径会重定价所有资产，因此声明本身往往比决议更能牵动市场。',
  'The written record of the last Fed meeting, three weeks after it.': '上次美联储会议的完整纪要，会后三周公布。',
  'It shows how split the committee was, which is what the next decision hangs on.': '它显示委员会分歧有多大，而这正是下次决议的关键。',
  'How fast consumer prices rose last month, headline and core.': '上月消费者物价的涨幅，包含整体与核心口径。',
  'Inflation sets the rate path, so a surprise here moves bonds before it moves stocks.': '通胀决定利率路径，因此意外读数先动债市、后动股市。',
  'The inflation gauge the Fed actually targets, with income and spending.': '美联储真正盯住的通胀指标，同时公布收入与支出。',
  'Same signal as CPI but on the Fed own measure, so it settles arguments CPI started.': '与 CPI 同类信号，但用的是美联储自己的口径，常用来给 CPI 引发的争论定音。',
  'Prices producers received last month — inflation one step upstream.': '上月生产者获得的价格 — 通胀链条上游一环。',
  'Producer prices lead consumer prices and feed several components of the Fed gauge.': '生产者物价领先消费者物价，并构成美联储口径的若干分项。',
  'The monthly jobs report: payrolls, unemployment, wages, and revisions.': '月度就业报告：新增就业、失业率、薪资与修正值。',
  'It is the cleanest read on growth, and wages feed straight back into the inflation debate.': '它是最干净的增长读数，而薪资又直接回到通胀讨论中。',
  'The first official estimate of last quarter economic growth.': '上季度经济增长的首次官方估算。',
  'Backward looking, but it settles whether the slowdown argument has any data behind it.': '虽是回顾性数据，却能验证“放缓论”是否有实据。',
  'What consumers spent at retailers last month, including the control group.': '上月消费者在零售端的支出，含核心控制组口径。',
  'Consumption is most of the economy, so the control group feeds the GDP tracking estimates.': '消费占经济大头，控制组数据直接进入 GDP 跟踪模型。',
  'A survey of US manufacturing: orders, employment, prices, production.': '美国制造业调查：订单、就业、价格与产出。',
  'Survey data turns before hard data, so it is an early read on the cycle.': '调查数据先于硬数据转向，是周期的早期信号。',
  'The same survey for services — the far larger half of the economy.': '同一套调查的服务业版本 — 经济中更大的那一半。',
  'Services inflation is the sticky part, so its prices component gets read as a rates signal.': '服务业通胀最具黏性，其价格分项常被当作利率信号解读。',
  'A consumer survey: current conditions, expectations, inflation expectations.': '消费者调查：现状、预期与通胀预期。',
  'The Fed watches whether households still believe inflation comes back down.': '美联储关注居民是否仍相信通胀会回落。',
  'Quarterly expiry of index futures, index options, and single-stock options.': '指数期货、指数期权与个股期权的季度集中到期。',
  'Dealer hedges unwind into the close, so volume and pinning matter more than news that day.': '做市商对冲在尾盘了结，当天成交与“钉价”比消息更重要。',
  'A central-bank symposium watched for policy signals from the speakers.': '央行研讨会，市场从发言中寻找政策信号。',
  'No release, but the framing of the speeches has moved the rate path before.': '没有数据发布，但讲话措辞曾多次撬动利率路径。',
  'A company reports the quarter and guides the next one.': '公司公布季度业绩并给出下季指引。',
  'Guidance sets the multiple; the peer group reprices with it whether it reported or not.': '指引决定估值倍数；无论是否发布业绩，同业都会随之重定价。',
  'A product or launch date you put on the calendar yourself.': '你自己加入日历的产品或发布日期。',
  'A conference or investor day you put on the calendar yourself.': '你自己加入日历的会议或投资者开放日。',
  'Dated company events move the name first and the peer group second.': '有明确日期的公司事件先影响个股，再扩散到同业。',
  'A spending or capacity date you put on the calendar yourself.': '你自己加入日历的支出或产能日期。',
  'Capex dates move the suppliers as much as the buyer.': '资本开支日期对供应商的影响不亚于买方。',
  'A policy, ruling, or regulatory date you put on the calendar yourself.': '你自己加入日历的政策、裁决或监管日期。',
  'Policy dates reprice a whole sector at once rather than one name.': '政策日期往往一次性重定价整个板块，而非单一个股。',
  'A macro date you put on the calendar yourself.': '你自己加入日历的宏观日期。',
  'Macro dates move the index complex and the rate path together.': '宏观日期会同时牵动指数体系与利率路径。',
  'A dated event with no shipped description.': '有日期但没有内置说明的事件。',
  'Unmapped, so the workspace watches the broad tape rather than guessing a sector.': '未做映射，因此工作台只跟踪大盘，而不臆测板块。',
  // My Portfolios (2026-08-20)
  'My Portfolios': '我的组合',
  'Portfolio name': '组合名称',
  ...
  'Record sell': '记卖出',
  'Still needed': '还需填写',
  'A-shares trade in lots of 100': 'A股一手为100股',
  'Live price': '现价',
  ...
  Analytics: '分析', 'choose cards': '选择卡片',
  'show all': '全部显示', '{n} hidden': '已隐藏 {n} 个',
  'Day contribution': '当日贡献', Breadth: '涨跌家数', Advancing: '上涨',
  Best: '最佳', Worst: '最差',
  ...
  'With cost': '已填成本', 'ex-cash': '不含现金', Trend: '净值走势', 'P&L map': '盈亏分布', since: '自', ccy: '币种', '{n}d': '{n}天', sh: '股',
