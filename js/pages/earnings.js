...
App.registerPage('earnings', function(container, data) {
    ...
    if (!earnings || !earnings.length) {
        container.innerHTML = '<div class="c-dim" style="padding:16px">No upcoming earnings data.</div>';
        return;
    }

    const sorted = [...earnings].filter(e => e.date !== 'N/A').sort((a, b) => (a.days_until ?? 999) - (b.days_until ?? 999));

    let html = '<div class="tt-section-title">Earnings Calendar</div>';
    ...
    for (const e of sorted) {
        ...
        if (e.days_until != null) {
            ...
            else if (e.days_until <= 30) cls = 'c-amber';
        }

        html += '<tr>';
        html += `<td>${Utils.symLink(e.symbol)}</td>`;
        html += `<td class="c-dim">${names[e.symbol] || ''}</td>`;
        html += `<td>${e.date}</td>`;
