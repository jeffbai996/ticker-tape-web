...
describe('opening entries', () => {
  it('the first trade on a hand-typed row files the row as an opening buy', async () => {
    const { addTxn, setHolding } = await import('../../src/lib/myPortfolios.js')
    const p = createPortfolio('刚', 'HKD')
    setHolding(p.id, '2628.HK', 1000, 25)
    addTxn(p.id, { d: '2026-08-24', sym: '2628.HK', side: 'buy', qty: 200, px: 28.5 })
    const book = loadPortfolios().find((x) => x.id === p.id)
    expect(book.txns.map((t) => [t.side, t.qty, t.px])).toEqual([['buy', 1000, 25], ['buy', 200, 28.5]])
    expect(book.txns[0].d < book.txns[1].d).toBe(true)
    const row = book.holdings.find((h) => h.symbol === '2628.HK')
    expect(row.shares).toBe(1200)
    expect(row.cost).toBeCloseTo((1000 * 25 + 200 * 28.5) / 1200, 3)
  })
  it('a costless hand row opens at the trade price; later trades add no second opening', async () => {
    const { addTxn, setHolding } = await import('../../src/lib/myPortfolios.js')
    const p = createPortfolio('刚', 'HKD')
    setHolding(p.id, '0700.HK', 100)
    addTxn(p.id, { d: '2026-08-24', sym: '0700.HK', side: 'sell', qty: 50, px: 450 })
    addTxn(p.id, { d: '2026-08-25', sym: '0700.HK', side: 'buy', qty: 10, px: 440 })
    const book = loadPortfolios().find((x) => x.id === p.id)
    expect(book.txns).toHaveLength(3)
    expect(book.txns[0]).toMatchObject({ side: 'buy', qty: 100, px: 450, d: '2026-08-23' })
    expect(book.holdings.find((h) => h.symbol === '0700.HK').shares).toBe(60)
  })
})
